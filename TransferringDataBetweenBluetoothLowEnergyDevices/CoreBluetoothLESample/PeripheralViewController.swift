...
class PeripheralViewController: UIViewController {
    ...
    override func viewDidLoad() {
        peripheralManager = CBPeripheralManager(delegate: self, queue: nil, options: [CBPeripheralManagerOptionShowPowerAlertKey: true,
                                                                                      CBPeripheralManagerOptionRestoreIdentifierKey: "myCentralManagerIdentifier"
        ])
        super.viewDidLoad()
...
//        }
//    }
        guard let _: AppDelegate = UIApplication.shared.delegate as? AppDelegate else { return }
//           appDelegate.showLocalNotification(id: "will restorestate peripheral manager",title: "",body: "Peripheral will restore state")
        ...
        if let service = services.first {
            ...
            if let characteristic = service.characteristics?.first as? CBMutableCharacteristic {
                ...
                for _ in characteristic.subscribedCentrals! {
//                    self.cbCentrals.append(subscribedCentral as! CBCentral)
                    guard let appDelegate: AppDelegate = UIApplication.shared.delegate as? AppDelegate else { return }
                       appDelegate.showLocalNotification(id: "will restorestate peripheral manager 2",title: "Peripheral will restore state",body: "")
                }
